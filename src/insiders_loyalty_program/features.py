...
def build_rfm_features(df, config: dict, apply_log: bool = False):
    """Aggregate raw transactions into one RFM row per customer.

    Parameters
    ----------
    apply_log : bool
        When True, apply log1p to all numeric RFM columns to reduce skewness.
        Set True only for standalone use; the sklearn Pipeline handles this
        automatically during model training.
    """
    import numpy as np
    import pandas as pd
    ...
    result = grouped.reset_index().rename(columns={customer_col: "customer_id"})

    if apply_log:
        num_cols = ["recency_days", "frequency", "monetary", "avg_ticket", "total_items"]
        for col in num_cols:
