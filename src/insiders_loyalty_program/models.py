...
def _rfm_preprocessor(X):
    """Preprocessing pipeline for RFM clustering: impute → log1p → scale."""
    import numpy as np
    from sklearn.compose import ColumnTransformer
    from sklearn.impute import SimpleImputer
    from sklearn.pipeline import Pipeline
...
def train_clustering(config: dict):
    ...
    # Use RFM-specific preprocessor (log1p + scale) instead of generic one
    preprocessor = _rfm_preprocessor(X) if config.get("project", {}).get("family") == "rfm_clustering" else _preprocessor(X)
    ...
    for k in cluster_range:
        model = KMeans(n_clusters=int(k), n_init="auto", random_state=seed)
    ...
    # Sort cluster labels by mean monetary value (0 = highest value = Insiders)
    if "monetary" in prepared.columns:
        mean_monetary = prepared.groupby("cluster")["monetary"].mean().sort_values(ascending=False)
        label_map = {old: new for new, old in enumerate(mean_monetary.index)}
        prepared["cluster"] = prepared["cluster"].map(label_map)
